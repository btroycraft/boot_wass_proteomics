...
ind_max = convert(Vector{Int}, select(DataFrame!(CSV.File("../data/ind_max_total.csv")), Not(:val))[1, :])
...
CSV.write("../data/ind_top_total_betti0.csv", hcat(DataFrame(val = map(x->x[1], max_list_betti0)), DataFrame(hcat(map(x->x[2], max_list_betti0)...)')))
...
CSV.write("../data/ind_top_total_betti1.csv", hcat(DataFrame(val = map(x->x[1], max_list_betti1)), DataFrame(hcat(map(x->x[2], max_list_betti1)...)')))
