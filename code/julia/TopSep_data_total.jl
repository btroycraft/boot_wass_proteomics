...
CSV.write("ind_top_total_betti0.csv", hcat(DataFrame(val = map(x->x[1], max_list_betti0)), DataFrame(hcat(map(x->x[2], max_list_betti0)...)')))
...
CSV.write("ind_top_total_betti1.csv", hcat(DataFrame(val = map(x->x[1], max_list_betti1)), DataFrame(hcat(map(x->x[2], max_list_betti1)...)')))
