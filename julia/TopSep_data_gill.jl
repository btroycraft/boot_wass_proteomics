...
max_list_total_betti0 = let
    ...
    (DIST_MAT_LIST, DIST_SUB_MAT_LIST) = top_sep_alloc(SUB_SIZE, GROUP_LIST, GROUP_VEC, DATA_MAT, RANGE_TOTAL)

    @everywhere workers() TOP_SEP = TopSepCall(0, $DIST_MAT_LIST, deepcopy($DIST_SUB_MAT_LIST))

    optim_subset_iter(x -> TOP_SEP(x), SUB_SIZE, length(RANGE_TOTAL);
        reps = REPS,
        type = "max",
        keep = KEEP,
        max_iter = MAX_ITER,
